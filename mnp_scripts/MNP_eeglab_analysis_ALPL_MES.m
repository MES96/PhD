...
eeglab_path = 'Q:\Toolboxes\eeglab14_1_1b'; % path to eeglab toolbox
fieldtrip_path = 'Q:\Toolboxes\fieldtrip-20180320'; % path to fieldtrip toolbox
batchfun_path = 'C:\Users\mes96\Documents\_generic_eeglab_batch\eeglab_batch_functions';
support_path = 'C:\Users\mes96\Documents\_generic_eeglab_batch\eeglab_batch_supporting_functions';
...
S.path.main = 'C:\MNP_Data\eeg\ana';
S.path.raw = 'C:\MNP_Data\eeg\raw'; % unprocessed data in original or BIDS format
...
S.path.datfile = 'C:\MNP_Data\eeg\participant_data_MES.xlsx'; % .xlsx file to group participants; contains columns named 'Subject', 'Group', and any covariates of interest
...
S.import.fname.parts = {'subject','block','cond','ext'}; % parts of the input filename separated by underscores, e.g.: {'prefix','study','group','subject','session','block','cond','suffix','ext'};
S.import.fname.partsep = {'_','_','',''}; % separator used in filename after each part (must be one per part even if empty)
...
S.import.study = {};
S.import.select.groups = {'FM'}; % either single/multiple groups, or use * to process all groups in the datfile
...
S.import.select.blocks = {'NLT','VLT'}; % blocks to load (each a separate file). Can use * as wildcard
S.import.select.conds = {'adapt','ALPL','ALPL_EEG'}; % conditions to load (each a separate file). Can use * as wildcard
...
S.prep.fname.partsep = {'_','_','_','',''}; % separator used in filename after each part (must be one per part even if empty)
...
S.prep.study = {};
S.prep.select.groups = {'FM'}; % either single/multiple groups, or use * to process all groups in the datfile
S.prep.select.subjects = {'MNP_002', 'MNP_003'}; % either single/multiple subjects, or leave blank to process all subjects in the datfile
S.prep.select.sessions = {}; % Can use * as wildcard
S.prep.select.blocks = {'NLT','VLT'}; % blocks to load (each a separate file). Can use * as wildcard
S.prep.select.conds = {'adapt','ALPL','ALPL_EEG'}; % conditions to load (each a separate file). Can use * as wildcard
...
S.prep.epoch.markers = {'S  1','S  2','S  3','S  4','S  5','S  6','S  7','S  8','S  9','S 10','S 11','S 12','S 13','S 14','S 15','S 16','S 17','S 18','S 19','S 20','S 21','S 22','S 23','S 24'};
...
S.prep.load.suffix = {'combined'}; % suffix to add to input file name, if needed. Can use * as wildcard was:combined
...
S.prep2.fname.parts = {'subject','suffix','ext'}; % parts of the input filename separated by underscores, e.g.: {'prefix','study','group','subject','session','block','cond','suffix','ext'};
...
S.prep2.select.groups = {'FM'}; % either single/multiple groups, or use * to process all groups in the datfile
S.prep2.select.subjects = {}; % either a single subject, or leave blank to process all subjects in folder
S.prep2.select.sessions = {};
S.prep2.select.blocks = {}; % blocks to load (each a separate file). Can use * as wildcard
S.prep2.select.conds = {}; % conditions to load (each a separate file). Can use * as wildcard
...
load(fullfile(S.path.main,'S'))
S.tf=struct;% clears the field
S.path.prep = 'C:\Data\MNP\eeg\ana\prep\separated'; % folder to load data from
S.tf.fname.parts = {'subject','block','cond','suffix','ext'}; % parts of the input filename separated by underscores, e.g.: {'prefix','study','group','subject','session','block','cond','suffix','ext'};
...
S.tf.study = {};
S.tf.select.groups = {'FM'}; % either single/multiple groups, or use * to process all groups in the datfile
...
S.tf.select.blocks = {'NLT','VLT'}; % blocks to load (each a separate file). Can use * as wildcard
S.tf.select.conds = {'ALPL','ALPL_EEG','adapt'}; % conditions to load (each a separate file). Can use * as wildcard
S.tf.load.suffix = {'epoched_cleaned'}; % suffix to add to input file name, if needed. Can use * as wildcard
...
S.ploterp.fname.parts = {'subject','block','cond','suffix','ext'}; % parts of the input filename separated by underscores, e.g.: {'prefix','study','group','subject','session','block','cond','suffix','ext'};
S.ploterp.study = {};
...
S.ploterp.select.groups = {'FM'}; % group index, or leave blank to process all 
